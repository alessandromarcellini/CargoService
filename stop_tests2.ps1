<#
================================================================================
 stop_tests2.ps1  -  tear down everything started by run_tests2.ps1 (Windows / PowerShell)
================================================================================
 Cleans up, in reverse order of run_tests2.ps1:

   1) the qak apps        (cargoservice TCP 8030 if any, robotsmart26 TCP 8020)
   2) the Gradle daemons  (gradlew --stop in the Sprint 2 and robotsmart26 dirs)
   3) the docker stacks   (vrWithGui26, then mosquitto)

 USAGE (from the repo root, in a SEPARATE window from run_tests2.ps1):
     powershell -ExecutionPolicy Bypass -File .\stop_tests2.ps1
   or simply:
     .\stop_tests2.ps1
...
# --- 1) stop the qak apps --------------------------------------------------
Write-Step 1 "Stopping the qak services (cargoservice TCP 8030 if any, robotsmart26 TCP 8020) ..."
